...
plot(y = train_set$beat_mkt, x = as.Date(index(train_set)), 
     ...
     xlab = "Date", ylab = "Probability", main = "SVM Prediction on training set")
points(y = svm_predtrain,  x = as.Date(index(train_set)), col = "red")
text(x = as.Date(index(train_set)[N_test/2]), y = max(1, svm_predtrain) + 0.1,
     labels = paste0("Train accuracy = ", round(train_accuracy, 5)), col = "blue")
